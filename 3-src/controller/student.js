...
module.exports.pagination = async function (req, res) {
  ...
  var start = 0;
  var studentPerPage = await Student.find(null, null, { skip: drop }).limit(perPage);
  var baseUrl = '?page=';
...
module.exports.search = async function (req, res) {
  ...
  const studentsSearch = await Student.find({ "name": { '$regex': q } }, null, { skip: drop }).limit(perPage);


  var start = 0;
  var numberPage = await Student.find({ "name": { '$regex': q } }).countDocuments() / perPage;
  // var numberPage = await Student.estimatedDocumentCount() / perPage;
