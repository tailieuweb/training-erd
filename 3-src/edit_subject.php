...
$id = $_GET['id'];
$subjectOb = $collectionSubject->findOne(['_id' => new MongoDB\BSON\ObjectID($id)]);

$name = $subjectOb['name'];
$subjectLevel = $subjectOb['subject_level'];
$message = '';
if(isset($_POST['name']) && isset($_POST['subject_level']))
{
  ...
  if(empty($name) || empty($subjectLevel)){
    ...
    if($collectionSubject->updateOne(['_id' => new MongoDB\BSON\ObjectID($id)], ['$set' => $document]));
    {
